...
func get_dominate_chance(identity: int) -> float:
	var base_chance: float = strength.get_value(identity) * 0.25
	base_chance -= intelligence.get_value(identity) * 0.125 + agility.get_value(identity) * 0.125
	if base_chance < 0.0:
		base_chance = 0.0
...
func get_coward_chance(identity: int) -> float:
	var base_chance: float = intelligence.get_value(identity) * 0.25
	base_chance -= strength.get_value(identity) * 0.125 + agility.get_value(identity) * 0.125
	if base_chance < 0.0:
		base_chance = 0.0
...
func get_reckless_chance(identity: int) -> float:
	var base_chance: float = agility.get_value(identity) * 0.25
	base_chance -= strength.get_value(identity) * 0.125 + intelligence.get_value(identity) * 0.125
	if base_chance < 0.0:
		base_chance = 0.0
